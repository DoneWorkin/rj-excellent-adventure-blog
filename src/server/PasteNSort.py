...
# Define a fixed thumbnail size for consistency
THUMBNAIL_SIZE = (150, 150)
# GRID_COLUMNS is now dynamic, removed as a constant
...
class ImageGridApp:
    def __init__(self, master):
        ...
        master.geometry("900x900+0+0") # Initial window size
        ...
        try:
            ...
            for filename in potential_filenames:
                ...
                if os.path.exists(filename) and os.path.isfile(filename):
                    ...
                    if ext.lower() in ['.png', '.jpg', '.jpeg', '.gif', '.bmp', '.tiff','.webm']:
    ...
    def close_application(self):
        ...
        """
        #self.fileBrowser.terminate()
        subprocess.Popen(['flatpak','kill','org.kde.dolphin'],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True  ) 
        
        #remove the line that tells dolphin that it closed improperly
        subprocess.run(["sed -i '/RestorePositionForNextInstance=false/d' ~/.var/app/org.kde.dolphin/.local/state/dolphinstaterc"],
